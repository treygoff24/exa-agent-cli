...
*Written 2026-08-03, in answer to a fair question from the Exa MCP maintainers. Numbers below are checked against the live Exa MCP docs (https://exa.ai/docs/reference/exa-mcp) as of that date and against `exa-agent capabilities --json` at v0.4.0.*

The Exa MCP is good at what it does. For a Claude Desktop or Cursor user who wants web search in their assistant, it is the right answer: one URL, OAuth, done. We built `exa-agent` because we kept wanting things from the Exa API that an MCP server, by its nature, is not positioned to give an autonomous coding agent. Three things, concretely.
...
This is a coverage gap, not a design flaw. The MCP is deliberately a search tool and its docs say so. Our agents needed the whole API.
...
If the MCP grows toward Websets, monitors, output caps, and self-description, we would be glad to see it. The more of this that is official, the better. Until then the two are complements, and we use both.

Trey Goff, `exa-agent-cli` (https://github.com/treygoff24/exa-agent-cli)
