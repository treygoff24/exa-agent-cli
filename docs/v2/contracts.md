...
- Ambiguous create failure (request sent, no confirmed response): exit non-zero, write a **pending-run record** (append-only JSONL under the state dir), and set `suggestedCommand` to the exact recovery (`exa-agent agent runs list --since ...` / re-issue with `--idempotency-key`).
- The pending-run record is an **agent-facing recovery contract** — agents parse it, so its shape is frozen. Schema `exa.cli.pending_run.v1`, one JSON object per line: `{ "schema": "exa.cli.pending_run.v1", "requestId": "...", "command": "agent runs create", "operationId": "agentRunsCreate", "apiPath": "/agent/runs", "idempotencyKey": null, "attemptedAt": "<SOURCE_DATE_EPOCH-aware>", "recoveryCommand": "exa-agent agent runs list --since ..." }`. Golden-pinned (§14).
...
- API keys, service keys, webhook secrets, and known secret headers are redacted in **all** output: stdout, stderr, `--trace` files, and `suggestedCommand`. `request.redacted: true` asserts it.
