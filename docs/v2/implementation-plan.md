...
- Live smoke (gated, budget-capped) covers read-only `search` and `contents`.
...
- Live smoke as the final gate (read-only, low-cost, cost-capped subset; see policy below).

**Acceptance (falsifiable):** every intent-mistake test green; **no scored ergonomics dimension below a stated floor** (e.g. ≥ 700 on the rubric's worst dimension — set the number when the first audit runs); all §14 goldens green; read-only live smoke green within budget.
...
- **Release blocks on the read-only, low-cost, budget-capped subset only** (Phase 6). Never exercise `admin` with a live service key in CI.
...
**`smoke.yml`** — nightly + dispatch, gated on the `EXA_API_KEY` secret: live smoke with `EXA_E2E=1`, budget cap, read-only low-cost subset by default. Not on every PR. Never `admin`.
