...
fn main() -> Result<()> {
    ...
    // `schema refresh` compares content, not bytes, so it also needs a hash that survives
    // reformatting. Build dependencies resolve serde_json without `preserve_order`, so
    // `serde_json::Map` is a `BTreeMap` here and `to_vec` already emits keys in sorted
    // order — the same canonical form `canonical_sha256` produces in the binary.
    let exa_value: serde_json::Value =
        serde_json::from_slice(&exa_bytes).context("parse exa spec as a JSON value")?;
    let spec_canonical_sha = hex(&Sha256::digest(
        serde_json::to_vec(&exa_value).context("re-serialize exa spec canonically")?,
    ));
    ...
    writeln!(
        out,
        "pub const EMBEDDED_SPEC_CANONICAL_SHA256: &str = {spec_canonical_sha:?};"
    )?;
