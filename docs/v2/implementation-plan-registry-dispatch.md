...
- **Seed the metadata Phase 1 consumes** (Codex F1/F3): exact `SecretCapture` rows for the 3 create ops (`admin keys create`→`apiKey` required; `monitor create`→`webhookSecret` optional; `websets webhooks create`→`secret` optional); `Confirm{protocol}` rows for every destructive op; **set `dangerous = true` for `websets-searches-cancel`** so `op.destructive()` includes it; `mixed_status_exit = true` for `contents`. Non-vacuous tests asserting these exact operationIds carry the expected metadata.
- **Canary** (invariant 4) with the grandfather list from the re-derived builder set.
**Exit gates (each a command+assertion):** `cargo xtask ci` green; contract test passes (every op has one consistent row); corpus regen diff empty; `assert REGISTRY has SecretCapture for {admin keys create, monitor create, websets webhooks create}`; `assert op("websets searches cancel").destructive() == true`; canary test green; capabilities-codegen reproducible.
**Review focus:** is the corpus derived from handlers (independent of the new fields)? Are additive fields genuinely unread? Is the seeded metadata exactly correct (the SecretCapture `required` split, the dangerous bit)?
...
- **Confirm-gate stage from `op.destructive()`** (3 protocols via the seeded `Confirm` rows). Non-vacuous regression test **naming `websets searches cancel`** refuses without confirmation (the fix only works because Phase 0 set its `dangerous` bit — native #3/Codex F3).
- **`SecretCapture` stage** (seeded rows; capture before redaction; honor `required`): delete `dispatch_monitor_create_live`, `dispatch_admin_keys_create_live`, `dispatch_websets_webhooks_create_live`. Non-vacuous "no secret to stdout" test over the 3 named ops.
